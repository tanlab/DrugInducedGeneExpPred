...
def build_model(input_dim, output_dim):
    gbm = RandomForestClassifier(n_estimators=150, class_weight='balanced')
    model = ClassifierChain(gbm, order='random', random_state=42)
...
for cell_line in ['VCAP', 'A549', 'A375', 'PC3', 'MCF7', 'HT29']:
    for target in ['up', 'dn']:
        for descriptor in ['ecfp', 'ecfp']:
            ...
            mlflow.log_param('Model Type', 'Multi-Task-RF')
            if target == 'up':
